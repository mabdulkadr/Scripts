...
$inputMethod = Read-Host "Enter '1' to input a device name, or '2' to upload a CSV file"
...
if ($inputMethod -eq '1') {
    ...
    Write-Host ""
    Write-Host "Invalid option selected. Please run the script again and select a valid option." -ForegroundColor Red
    exit
}
